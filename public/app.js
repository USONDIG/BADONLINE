...
function mountBadonline(root, send, result = null) {
  ...
  const studio = mountStudio(root, listen);
  ...
  root.querySelectorAll('a[href^="#"]').forEach(link => listen(link,'click',e => {
    ...
    if (link.dataset.example || link.dataset.openStudio) studio?.open(link.dataset.openStudio || 'shirt',link.dataset.example || '');
  ...
  const show = reply => {status.textContent=reply.message;button.disabled=false;if(reply.ok){form.reset();studio?.clearCreations();}};
  ...
  listen(form,'submit',async e=>{
    ...
    const data=Object.fromEntries(new FormData(form));data.request_id=crypto.randomUUID();data.creations=studio?.creations() || [];
...
mountBadonline(document,async data=>{
  const response=await fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data),signal:AbortSignal.timeout(40000)});
